...
// reHTMLComment matches writer notes (arc tags, point lines, verify tags).
// Tested 2026-07-17 (see build/comment_test.*): pandoc passes HTML comments
// through into EPUB and HTML output, so writer notes must be stripped here,
// before template execution and rendering.
var reHTMLComment = regexp.MustCompile(`(?s)<!--.*?-->`)

// stripComments removes HTML comments from chapter content.
// Caveat: do not place a literal <!-- --> inside fenced code blocks; the
// stripper does not parse code fences.
func stripComments(b []byte) ([]byte, int) {
	n := len(reHTMLComment.FindAll(b, -1))
	if n == 0 {
		return b, 0
	}
