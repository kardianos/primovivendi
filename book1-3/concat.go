...
func run() error {
	chdir := flag.String("chdir", "chapters", "chapter directory")
	outputFileOpt := flag.String("o", "book.md", "output file")
	includeExtOpt := flag.String("ext", ".md", "only includ extension")
	flag.Parse()
	...
	for _, e := range entries {
		name := e.Name()
		if len(includeExt) > 0 && !strings.HasSuffix(name, includeExt) {
			continue
		}
		if e.IsDir() || strings.HasSuffix(name, "_exp.md") {
			continue
		}
